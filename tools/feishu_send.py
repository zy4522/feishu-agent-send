#!/usr/bin/env python3
"""
feishu_send.py - 发送消息给飞书 Agent v3.0.0
...
from feishu_agent_send import AgentConfig, feishu_agent_send, feishu_agent_send_and_deliver, list_known_agents
...
def main():
    ...
    if not from_agent or not my_chat_id:
        ...
        print(json.dumps({
            ...
            'error': '缺少发送者信息',
            'hint': hint,
            'v3.0.0_help': '首次使用请运行：python3 feishu_set_self.py <你的Agent名> <你的chat_id>'
        }, ensure_ascii=False))
        sys.exit(1)
    ...
    if not agent_info:
        ...
        print(json.dumps({
            ...
            'v3.0.0_help': f'请先添加：python3 feishu_add.py {to_agent} oc_xxx'
        }, ensure_ascii=False))
        sys.exit(1)
    ...
    if deliver_mode:
        ...
        result = {
            ...
            'v3.0.0_hint': '使用 --deliver 获取完整发送指令和日志记录提示'
        }
        print(json.dumps(result, ensure_ascii=False, indent=2))
        if multi_scene:
            print(f"\n💡 提示：如需发到其他场景，请使用 --chat-type 参数")
